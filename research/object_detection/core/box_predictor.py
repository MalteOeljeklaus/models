...
class ConvolutionalBoxAnglePredictor(BoxPredictor):
  """Convolutional Box and Angle Predictor.
  ...
  def __init__(self,
    ...
    Raises:
      ValueError: if min_depth > max_depth.
    """
    super(ConvolutionalBoxAnglePredictor, self).__init__(is_training, num_classes)
    if min_depth > max_depth:
      raise ValueError('min_depth should be less than or equal to max_depth')
    self._conv_hyperparams = conv_hyperparams
    self._min_depth = min_depth
    ...
    with slim.arg_scope(self._conv_hyperparams), \
      ...
      with slim.arg_scope([slim.conv2d], activation_fn=None,
        ...
        class_predictions_with_background = slim.conv2d(
            ...
            biases_initializer=tf.constant_initializer(
                self._class_prediction_bias_init))
        angle_predictions = slim.conv2d(
            net, num_predictions_per_location,
            [self._kernel_size, self._kernel_size], scope='AnglePredictor',
            biases_initializer=tf.constant_initializer(
                self._class_prediction_bias_init))
        if self._apply_sigmoid_to_scores:
          class_predictions_with_background = tf.sigmoid(
              class_predictions_with_background)
          angle_predictions = tf.sigmoid(
              angle_predictions)
