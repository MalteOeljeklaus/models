...
def prepare_example(image_path, annotations, label_map_dict):
  ...
  if not os.path.isfile(os.path.join(dirname, 'seg_'+filename)):
      ...
      with tf.gfile.GFile(os.path.join(dirname, 'seg_'+filename), 'rb') as fid:
        seg_encoded_png = fid.read()
      seg_encoded_png_io = io.BytesIO(seg_encoded_png)
      seg_image = pil.open(seg_encoded_png_io)
